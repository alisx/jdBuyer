...
import re
# import logging
from log.jdlogger import logger as logging
...
class JDSpider:
    ...
    def __init__(self):
        ...
        self.cookies = {
                  ...
                  # RGJTJMDTK4RFS7LHXXSCWXZ3L5ILNF7UOSW6XCW4IO6QA3WVLBOTSVPUKGTV722T6ICQASLUC5FMRHXDSJEXIAHHBM
        self.eid = 'DHPVSQRUFPP6GFJ7WPOFDKYQUGQSREWJLJ5QJPDOSJ2BYF55IZHP5XX3K2BKW36H5IU3S4R6GPU7X3YOGRJGW7XCF4'
                 # 900a4febb112e98885370a4e838201ba
    ...
    def checkLogin(self):
        ...
        # checkUrl = 'https://passport.jd.com/user/petName/getUserInfoForMiniJd.action'
        ...
        try:
            ...
            with open('cookie', 'rb') as f:
                ...
                cart_info = self.cart_detail(cookies= cookies)
                # return True
                # print(cart_info)
                if cart_info:
                    print('登录成功!')
                ...
                # response = requests.get(checkUrl, params={
                #     'callback': 'jsonpUserinfo',
                #     '_': int(time.time() * 1000)
                # },cookies=cookies, headers=headers)
                # response = requests.get(checkUrl, cookies=cookies)
                # with open('userinfo2.html', 'w') as f:
                #     f.write(response.text)
                # soup = BeautifulSoup(response.text, "lxml")

                # # if not soup.find('dev', class_='login-form'):
    ...
    def good_detail(self, skuId, area_id):
        ...
        try:
            ...
            good_data['name'] = name
            # 购物车链接
            cart_link = soup.find("a", id="InitCartUrl")
            if cart_link:
                cart_link = cart_link['href']
            ...
            # 预约链接
            reserv_link = soup.find("a", id="btn-reservation")
            if reserv_link:
                reserv_link = reserv_link['href']
                if reserv_link != '#none':
                    if reserv_link[:2] == '//':  # '//cart.jd.com/gate.action?pid=5504364&pcount=1&ptype=1'
                        reserv_link = 'https:' + cart_link
                    good_data['reserv_link'] = reserv_link
        ...
        print(f"预约链接：{good_data['reserv_link']}")
        return good_data
    ...
    def buy(self, options):
        ...
        try:
            ...
            if options.good in cart_info_dict.keys():  # 如果之前添加过购物车了
                ...
                # cart_link='https://cart.jd.com/addToCart.html?rcd=1&pid=3035477&pc=1&eb=1&rid='+str(time.time()*1000)+'&em='
        
                response = self.sess.get(cart_link, cookies=self.cookies, allow_redirects=False)
                soup = BeautifulSoup(response.text, "lxml")
                cartDetail = self.cart_detail()
                if cartDetail.get(options.good, None):
                    print("商品已成功加入购物车！")
                else:
                    print("添加到购物车失败")
                    return False
    ...
    def cart_detail(self, cookies=None):
        # get info of cart
        cart_url = 'https://cart.jd.com/cart.action'

        response = self.sess.get(cart_url, cookies= cookies if cookies else self.cookies)
    ...
    def reserve(self, pid):
        ...
        payload = {}
        res = self.sess.get(url, params={
            'callback': 'fetchJSON',
            'sku': pid,
            '_': int(time.time() * 1000)
        }, cookies=self.cookies)
        assert res.status_code == 200
        ret = json.loads(res.text[10:-2])
        if not ret.get('url', None):
            return True

        rurl = 'https:' + ret['url']
        rres = self.sess.get(rurl, cookies= self.cookies)
        assert res.status_code == 200

        soup = BeautifulSoup(rres.text, "lxml")
        result = soup.find("p", class_="bd-right-result").text.strip()
        print("reserv result:", result)
        if result.find('预约成功') > -1 or result.find('已成功预约') > -1:
            return True
        else:
            return False
...
class Conf:
    def __init__(self, area='1_2808_51531', good='100005171461', 
    count=1, wait=1000, flush=True,submit=False,timer=''):
        self.area = area
        self.good = good
        self.count = count
        self.wait = wait
...
if __name__ == '__main__':
    ...
    # runner = Runner(1, spider, options)
    
    # signal.signal(signal.SIGINT, over)
    # signal.signal(signal.SIGTERM, over)
    ...
    # for test
    # options.count = 1
    options.good = '45332147740'
    # options.area = '1_2808_51531'
    # options.timer = "21:00:02"

    spider = JDSpider()
    if not spider.checkLogin():
        if not spider.login_by_QR():
            sys.exit(-1)
    # print(spider.sess.get('https://passport.jd.com/uc/login?ltype=logout', cookies=spider.cookies).status_code)
    # print(spider.cart_detail())
    # print(spider.good_stock(options.good, options.area))
    # while True:
    #     if should_monitor(options.timer):
    #         if spider.reserve(options.good):
    #             print('预约成功')
    #             break
    #     time.sleep(1)

    # print(spider.sess.get('https://passport.jd.com/uc/login?ltype=logout', cookies=spider.cookies).status_code)
    # print(spider.sess.get('https://home.jd.com', cookies=spider.cookies).text)
    # spider.good_detail(options.good, options.area)
    while True:
        if should_monitor(options.timer):
...
# 100011551632 15点预约 20点抢
# 65624145328  10点预约 15点抢
# 65708238590  需要关注
# 65698681192 需要关注
# 65425149164
# 65420816727
# 65420816726
# 65533383512
# https://cart.jd.com/addToCart.html?rcd=1&pid=100005171461&pc=1&eb=1&rid=1582720319685&em=